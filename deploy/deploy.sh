...
sshpass -p $pw ssh $user@l040101-ws01.ua.pt "nohup java -cp $jar heist.distributed.test.LoggerServerTest > /dev/null 2>&1 &"

#l040101-ws03|23292|controlCollection
echo "Starting ControlCollection Site Server"
sshpass -p $pw ssh $user@l040101-ws03.ua.pt "nohup java -cp $jar heist.distributed.test.ControlCollectionSiteServerTest > /dev/null 2>&1 &"

#l040101-ws04|23293|concentration
echo "Starting Concetration Site Server"
sshpass -p $pw ssh $user@l040101-ws04.ua.pt "nohup java -cp $jar heist.distributed.test.ConcentrationSiteServerTest > /dev/null 2>&1 &"
